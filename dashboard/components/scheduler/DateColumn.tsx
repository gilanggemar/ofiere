...
export const DateColumn = memo(function DateColumn({
    ...
    return (
        ...
        >
            ...
            >
                <div className={cn(
                    'nerv-caption uppercase',
                    isToday ? 'text-[var(--accent-base)] font-semibold' : 'opacity-50',
                )}>
                    {dayName}
                </div>
                <div className={cn(
                    isToday ? 'nerv-metric-md text-[var(--accent-base)]' : 'nerv-body font-medium',
                )}>
                    {dayNum}
                </div>
                {(isMonthStart || isToday) && (
                    <div className="nerv-caption mt-0.5 opacity-60">{monthLabel}</div>
                )}
            ...
            <div className="flex-1 overflow-y-auto p-1.5 space-y-0">
                ...
                {events.length > 5 && (
                    <div className="nerv-caption text-center py-1 opacity-60">
            ...
            <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-150 p-1 shrink-0">
                <button
                    ...
                    className="w-full flex items-center justify-center gap-1 py-1 rounded-md text-[var(--accent-base)] hover:bg-[var(--accent-base)]/10 transition-colors nerv-caption"
                >
